...
test('editor: persisted content, media, SEO, invalid input, stale edit and exact restore',async({page})=>{
  ...
  const beforePath=output('content-before.json');
  ...
  fs.writeFileSync(output('content-restored.json'),JSON.stringify(restored,null,2)+'\n');
  fs.writeFileSync(output('editor-results.json'),JSON.stringify({beforeSha256:hash(before.content),changedSha256:hash(changed.content),restoredSha256:hash(restored.content),themeEntryUnchanged:true,invalidSaveStatus:422,staleSaveStatus:409,restored:true},null,2)+'\n');
});
