...
The local environment is noindex and binds only loopback. HOME-001, PRODUCT-000 and shared navigation/footer/menu/cookie UI are implemented. PRODUCT-000 intentionally does not create its fourteen Grade pages, two Process pages, Applications, Documents, Markets or the RFQ receiver; those routes remain genuine external 404 dependencies and Hub actions fail closed except for always-visible clean RFQ links. No analytics or optional-consent storage is active. Production deployment and indexing remain separate work.

See `docs/handoffs/HOME-001-gate8.md` for the historical homepage candidate and `docs/handoffs/PRODUCT-000-gate8.md` for the Products candidate. Keep the applicable candidate runtime unchanged until Gate9 PASS/RETURN.
