...
需求明确 → 接收设计与内容 → 拆分任务 → 分支开发 → 本地验证 → 分项提交 → PR 与代码审查 → 独立页面验收 → 合并归档。
...
| 开发 | 从明确基线建功能分支，按任务实现 | 代码与内容分离，遵守本站数据边界；不夹带其他需求 |
| 本地验证 | 运行与变更相关的检查，检查实际页面或后台行为 | 有真实结果；临时编辑已恢复；失败原因与未测项如实记录 |
| 分项提交 | 检查差异，提交完成且已验证的工作项 | 一个独立完成项一个 commit，提交信息说明目的 |
| PR 与审查 | 推送分支、创建 PR，执行自动检查并审查代码 | 必修问题关闭；记录检查结果、影响、数据变化和剩余依赖 |
| 独立验收 | 对照批准输入检查固定候选的页面、内容与交互 | 结论绑定实现版本、内容与环境；自检不能替代独立验收 |
| 合并归档 | 按当前任务授权合并，保存验收结论及待办归属 | 主分支保留可追溯历史；发布是否进行另行记录 |

没有新增风险、改动或失败，不反复重跑已经通过的相同检查。验收有明确 Finding 时，只针对该问题修复和回归。

## 2. 分支和提交

- `main`：集成主分支；后续常规开发通过 PR 合入。
- `codex/<任务>-<简述>`：功能分支，例如 `codex/home-001-wordpress`。
- `codex/fix-<简述>`、`codex/docs-<简述>`：修复或文档工作可单独开分支。
- 当前远端尚未初始化。首次同步需要单独落实基线和默认分支，不按已有远端 `main` 处理，也不强推覆盖历史。
...
commit 是本地记录；push 是同步到 GitHub；merge 是合入主分支；deploy 是更新运行站点。四个动作分别报告，不把“已提交”写成“已发布”。不随意 squash 已按完成项保存的提交；确需整理历史时在 PR 中说明。
