...
function tio2_products_page_is_owned(WP_Post $page): bool {
    $slug_owned = $page->post_name === 'products';
    if ($page->post_status === 'trash') {
        $slug_owned = $page->post_name === 'products'
            || ($page->post_name === 'products__trashed'
                && get_post_meta($page->ID, '_wp_desired_post_slug', true) === 'products');
    }
    return $page->post_type === 'page'
        && $slug_owned
        && get_post_meta($page->ID, '_tio2_managed_page', true) === '1'
...
function tio2_products_managed_page(): ?WP_Post {
    ...
    if (!$page) return null;
    if (!tio2_products_page_is_owned($page)) throw new RuntimeException('The products slug is already owned by another page.');
    return $page;
}
...
function tio2_products_migrate(): array {
    ...
    $created_content_now = false;
    $created_page_id_now = 0;
    try {
        ...
        if (!$page) {
            ...
            $created_page_id_now = (int) $page_id;
        ...
        if ($created_page_id_now > 0) wp_trash_post($created_page_id_now);
        if ($created_content_now) delete_option(TIO2_PRODUCTS_OPTION);
...
function tio2_products_rollback(): array {
    ...
    if ($page && !tio2_products_page_is_owned($page)) {
        throw new RuntimeException('Products rollback refused a page ownership mismatch.');
    }
    if ($page && $page->post_status !== 'trash') {
        if (!wp_trash_post($page_id)) throw new RuntimeException('Products page could not be moved to trash.');
    }
...
function tio2_products_resume(): array {
    ...
    $page = $page_id ? get_post($page_id) : null;
    if ($page && !tio2_products_page_is_owned($page)) {
        throw new RuntimeException('Products resume refused a page ownership mismatch.');
    }
    if ($page && $page->post_status === 'trash') {
        $slug_owner = get_page_by_path('products', OBJECT, 'page');
        if ($slug_owner && (int) $slug_owner->ID !== $page_id) {
            throw new RuntimeException('Products resume refused because the products slug is occupied.');
        }
    }
    delete_option(TIO2_PRODUCTS_SUSPENDED_OPTION);
    delete_option(TIO2_PRODUCTS_ERROR_OPTION);
    if ($page) {
        if ($page->post_status === 'trash' && !wp_untrash_post($page_id)) {
            throw new RuntimeException('Products page could not be restored from trash.');
        }
