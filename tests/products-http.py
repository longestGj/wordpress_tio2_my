...
EVIDENCE = Path(os.environ.get("PRODUCT_EVIDENCE_DIR", Path(RUNTIME["output_dir"]) / "products-http"))
...
assert home.select_one('link[rel="canonical"]')["href"] == PUBLIC_URL
assert home.select_one('meta[property="og:url"]')["content"] == PUBLIC_URL
...
products_url = PUBLIC_URL + "products/"
assert products.select_one('link[rel="canonical"]')["href"] == products_url
assert products.select_one('meta[property="og:url"]')["content"] == products_url
...
assert BASE_URL not in scripts[0].string
